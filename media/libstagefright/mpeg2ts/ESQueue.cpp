...
static unsigned parseDTSSyncFrame(
    ...
    if(syncValue == DTS_SYNCWORD_CORE) {
        ...
        ALOGV("DTSHD core sub stream frame size = %d", frameByteSize);
        ...
        payloadSize = frameByteSize;
        ALOGV("DTSHD extension sub stream info: frame size = %d", frameByteSize);
        ...
        while(bits.numBitsLeft() >= 32){
            ...
            if(bits.getBits(32) == DTS_SYNCWORD_LBR) {
                ...
                if(lbrHdrType == 2){
                    ...
                    unsigned short lbrChannelMask = bits.getBits(16);
                    ...
                    ALOGV("DTSHD extension sub stream info: lbrChannelMask=%04x", lbrChannelMask);
                    dtsChannelCount = 0;
                    ...
                    ALOGV("DTSHD extension sub stream info: dtsSamplingRate=%d, dtsChannelCount=%d", dtsSamplingRate, dtsChannelCount);
                    break;
                }
            }
        }
