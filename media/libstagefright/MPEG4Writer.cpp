...
off64_t MPEG4Writer::addLengthPrefixedSample_l(MediaBuffer *buffer, size_t maxBytesToSearch) {
    ...
    while (void *accessUnitPtr = memmem(static_cast<char *>(copy->data()) + copy->range_offset(),
        ...
        ALOGV("Found next SC, current NAL offset %u, length %u",
                accessUnit->range_offset(), accessUnit->range_length());
...
MPEG4Writer::Track::Track(
     ...
     * Add WA for some platform which will generate multi-slice frames.
     */
    ...
    if (mIsAvc &&
        ...
        if (width * height > 1280 * 720) {
            ...
            ALOGD("HW limitation on 8956 chip, enable full SC search on \
multi-slice frames for clips above 720p");
        }
    }
...
////////////////////////////////////////////////////////////////////////////////
        ...
        if (!hasMultipleTracks) {
            off64_t offset = (mIsAvc | mIsHEVC) ? mOwner->addLengthPrefixedSample_l(copy,
                        mMaxBytesToSearch) : mOwner->addSample_l(copy);
