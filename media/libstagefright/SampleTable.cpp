...
status_t SampleTable::setSampleDescParams(uint32_t count, off64_t offset, size_t size)
{
    ...
    for(uint32_t i = 0; i < count; ++i) {
        ...
        ALOGV("parsing chunk %c%c%c%c", ((char *)&avc1_chunk_type)[3],
                                                   ((char *)&avc1_chunk_type)[2],
                                                   ((char *)&avc1_chunk_type)[1],
                                                   ((char *)&avc1_chunk_type)[0]);

        if (avc1_chunk_type != FOURCC('a', 'v', 'c', '1')) {
            ALOGE("Multiple Non AVC Sample Entries are not supported");
            return ERROR_MALFORMED;
        }
        ...
        ALOGE("data_ref_index : %d width : %d height: %d", data_ref_index, width, height);
        ...
        if((avcc_chunk_size == 0)|| (avcc_chunk_size == 1)) {
            ALOGE("chunk size error while reading avCC atom");
            return ERROR_MALFORMED;
        }

        ALOGV("parsing chunk %c%c%c%c",
        ...
        if (avcc_chunk_type != FOURCC('a', 'v', 'c', 'C')) {
            ALOGE("'avcC' atom expected, but not found");
            return ERROR_MALFORMED;
        }
