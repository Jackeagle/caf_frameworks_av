...
status_t MPEG4Extractor::parseChunk(off64_t *offset, int depth) {
    ...
    ALOGV("parsing chunk %c%c%c%c at depth %d", ((char *)&chunk_type)[3], ((char *)&chunk_type)[2], ((char *)&chunk_type)[1], ((char *)&chunk_type)[0], depth);
            ...
            if (entry_count > 1) {
                ...
                if (!strcasecmp(mime, MEDIA_MIMETYPE_TEXT_3GPP)) {
                    ...
                    // For now we only support a single type of media per track.
                     ...
                     ALOGI("This clip has multiple avcc atoms ");
    ...
    *out = NULL;
    ...
    if ((sampleDescIndex > 1) && (mCurrentSampleDescIndex != sampleDescIndex)) {
        ...
        if(mSampleTable->getSampleDescAtIndex(mCurrentSampleDescIndex, &avccData, &avccSize) != OK) {
           ALOGE("Error while reading avcC data at index (%d)", mCurrentSampleDescIndex);
           return ERROR_MALFORMED;
        }
        const void *data;
        ...
        CHECK_EQ((unsigned)ptr[0], 1u);  // configurationVersion == 1
        mNALLengthSize = 1 + (ptr[4] & 3);
        ALOGV("mNALLengthSize is Updated to the new value %d",mNALLengthSize);
    }
    ...
    if (!mIsAVC || mWantsNALFragments) {
        ...
        *out = clone;
        ...
        if (usesDRM) {
            ...
            if(insertAVCC == true) {
                ALOGV("MultiCodec: Updating the new AVCC atom");
