...
status_t AACDecoder::initCheck() {
    ...
    if (meta->findData(kKeyESDS, &type, &data, &size)) {
        ...
        if (PVMP4AudioDecoderConfig(mConfig, mDecoderBuf)
                != MP4AUDEC_SUCCESS) {
            ALOGE("Error in setting AAC decoder config");
    ...
    if (meta->findData(kKeyAacCodecSpecificData, &type, &data, &size)) {
       if( size > AAC_MAX_FORMAT_BLOCK_SIZE ) {
          ALOGE("AAC FormatBlock is too big %d", size);
          return ERROR_UNSUPPORTED;
       }
    ...
    *out = NULL;
    ...
    if (mInputBuffer == NULL) {
        ...
        if (  mTempBufferDataLen ) {
            ALOGV("Incomplete frame assembly is in progress mTempBufferDataLen %d", mTempBufferDataLen);
            if ( mTempBufferDataLen + inputBufferSize > mTempBufferTotalSize ) {
                ALOGE("Temp buffer size exceeded %d input size %d", mTempBufferTotalSize, inputBufferSize);
                return UNKNOWN_ERROR;
            }
            ...
            if ( inputBufferSize + mTempBufferDataLen < mInputBufferSize ) {
                ALOGV("Reached end of stream case" );
    ...
    ALOGV("Input Buffer Length %d Offset %d size %d", mInputBuffer->range_length(), mInputBuffer->range_offset(), mInputBufferSize);
    ...
    ALOGV("AAC decoder %d frame length %d used length %d ", decoderErr, inputBufferSize, mConfig->inputBufferUsedLength);
    if( inputBufferSize < mConfig->inputBufferUsedLength ) {
        ALOGE("unexpected error actual len %d is less than used len %d", inputBufferSize, mConfig->inputBufferUsedLength);
    ...
    if ( decoderErr == MP4AUDEC_INCOMPLETE_FRAME  && aacformattype == true) {
        ALOGW("Handle Incomplete frame error inputBufSize %d, usedLength %d", inputBufferSize, mConfig->inputBufferUsedLength);
        if(mConfig->inputBufferUsedLength == mInputBufferSize){
           ALOGW("Decoder cannot process the buffer due to invalid frame");
           decoderErr = MP4AUDEC_INVALID_FRAME;
        } else {
            if ( !mTempInputBuffer ) {
                ...
                mTempBufferDataLen = 0;
                if (mTempInputBuffer == NULL) {
                   ALOGE("Could not allocate temp buffer bytesToAllocate quit playing");
                   return UNKNOWN_ERROR;
                }
                mTempBufferTotalSize = bytesToAllocate;
                ALOGV("Allocated tempBuffer of size %d data len %d", mTempBufferTotalSize, mTempBufferDataLen);
            }
            ...
            if (mTempBufferDataLen != 0) {
                ...
                ALOGV("Appending remaining data tempDataLen %d usedLength %d", mTempBufferDataLen, mConfig->inputBufferUsedLength);
            ...
            ALOGV("mTempBufferDataLen %d inputBufferUsedLength %d", mTempBufferDataLen, mConfig->inputBufferUsedLength);
            ...
            if ( mTempBufferDataLen >= mInputBufferSize ) {
                ALOGV("mTempBufferDataLen %d exceeded mInputBufferSize %d ", mTempBufferDataLen, mInputBufferSize);
