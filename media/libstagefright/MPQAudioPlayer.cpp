...
void MPQAudioPlayer::pause(bool playPendingSamples) {
    ...
    if(!mPauseEventPending) {
        ...
        mQueue.postEventWithDelay(mPauseEvent, MPQ_AUDIO_PAUSE_TIMEOUT_USEC);
...
void MPQAudioPlayer::resume() {
    ...
    if (!mIsAudioRouted) {
        ALOGV("Opening a session for MPQ Audio playback");
        audio_output_flags_t flags = (audio_output_flags_t) (AUDIO_OUTPUT_FLAG_TUNNEL |
                                                            AUDIO_OUTPUT_FLAG_DIRECT);
    ...
    while (1) {
        ...
        if (mInputBuffer == NULL) {
                  ...
                  || (err != OK && mInputBuffer == NULL));
            {
                ...
                mPositionTimeRealUs = mSeekTimeUs + getAudioTimeStampUs() + ((int64_t)mAudioSink->latency() * 1000);
...
int64_t MPQAudioPlayer::getRealTimeUs() {
    ...
    uint64_t realTimeUs = 0;
    switch(mDecoderType) {

        case EHardwareDecoder:
        case EMS11Decoder:
        case ESoftwareDecoder:
            realTimeUs = mSeekTimeUs + getAudioTimeStampUs();
            break;
        default:
            ALOGV("Invalid Decoder return zero time");
            realTimeUs = 0;
            break;
    }
    return realTimeUs;
...
int64_t MPQAudioPlayer::getMediaTimeUs() {
    ...
    switch(mDecoderType) {
        case EHardwareDecoder:
        case ESoftwareDecoder:
            if(mIsPaused) return mTimePaused;
            else return mSeekTimeUs + getAudioTimeStampUs();
        case EMS11Decoder:
...
bool MPQAudioPlayer::getMediaTimeMapping(
    ...
    switch(mDecoderType) {
        case EHardwareDecoder:
        case ESoftwareDecoder:
            *mediatime_us = *realtime_us = mSeekTimeUs + getAudioTimeStampUs();
            mPositionTimeRealUs = mPositionTimeMediaUs = *realtime_us;
            break;
        case EMS11Decoder:
...
int64_t MPQAudioPlayer::getAudioTimeStampUs() {
    ...
    if (mIsAudioRouted == false || mAudioSink.get() == NULL) {
        // Possible in case of pauseTimeOut
        ALOGW("Audio is not routed or AudioSink is NULL !!!");
        return 0;
    }
...
void MPQAudioPlayer::onPauseTimeOut() {
    ...
    Mutex::Autolock autoLock(mLock);
    if (!mPauseEventPending) {
        ALOGV("onPauseTimeOut: EventExpired");
        return;
    }
    mPauseEventPending = false;
    // 1.) Set seek flags
