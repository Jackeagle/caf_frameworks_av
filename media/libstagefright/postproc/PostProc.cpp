...
status_t PostProc::init(sp<MediaSource> source, sp<PostProcNativeWindow> nativeWindow, const sp<MetaData> &meta, char *name)
{
    Mutex::Autolock autoLock(mLock);
    mDerivedName = name;
    POSTPROC_DERIVED_LOGV("%s:  begin", __func__);
    ...
    if (err != OK) {
        POSTPROC_DERIVED_LOGE("Set buffer info failed\n");
        return err;
    }

    err = getInputFormat();
    if (err != OK) {
        POSTPROC_DERIVED_LOGE("Get input format failed\n");
        return err;
    }
...
status_t PostProc::start(MetaData *params)
{
    ...
    if (mState == PAUSED) {
        ...
        if (err != OK) {
            POSTPROC_DERIVED_LOGE("source start failed while in paused failed\n");
            return err;
        }
        mState = EXECUTING;
    ...
    if(mNativeWindow != NULL) {
        ...
        if (err != OK) {
            POSTPROC_DERIVED_LOGE("Notify native window failed\n");
            return err;
        }
    }
    ...
    err = mSource->start(params);
    if (err != OK) {
        POSTPROC_DERIVED_LOGE("source start failed\n");
        return err;
    }
    ...
    if (mNativeWindow == NULL) {
        ...
        if (err != OK) {
            POSTPROC_DERIVED_LOGE("Could not allocate post proc buffers\n");
            return err;
        }
    }
...
status_t PostProc::stop()
{
    ...
    POSTPROC_DERIVED_LOGV("Joining the worker thread\n");
    destroyWorkerThread();
    ...
    POSTPROC_DERIVED_LOGV("Stopping source");
    return mSource->stop();
}
...
status_t PostProc::read(MediaBuffer **buffer, const ReadOptions *options)
{
    ...
    if (options && options->getSeekTo(&seekTimeUs, &seekMode)) {
        POSTPROC_DERIVED_LOGV("ppread:  seeking");
    ...
    if (mState == ERROR) {
        if (mReadError == INFO_FORMAT_CHANGED) {
            POSTPROC_DERIVED_LOGE("Info format changed not supported\n");
            return UNKNOWN_ERROR;
        }
        return mReadError;
    }

    POSTPROC_DERIVED_LOGV("ppread: Got an output buffer\n");
    ...
    *buffer = outBuffer->buffer;
    ...
    if ((mLastBufferFormat != ppBufFormat) && (mNativeWindow != NULL)) {
        POSTPROC_DERIVED_LOGV("update buffers geometry with old format : %x, new format :%x, stride : %d, slice : %d \n", mLastBufferFormat, ppBufFormat, mStride, mSlice);
...
status_t PostProc::pause()
{
    ...
    POSTPROC_DERIVED_LOGV("Signalling post proc buffer condition\n");
    mPostProcBufferCondition.signal();
    POSTPROC_DERIVED_LOGV("now calling source pause\n");
...
status_t PostProc::signalMessage(PostProcMessage *msg)
{
    ...
    if (!mPostProcessingEnabled) {
        POSTPROC_DERIVED_LOGV("Post Processing disabled\n");
...
void PostProc::readLoop()
{
    ...
    while (mDone != true) {
        ...
        ReadOptions optionsCopy;
        {
            ...
            if (mState == SEEKING) {
                POSTPROC_DERIVED_LOGV("ppreadloop:  got seeking before source read");
                mState = EXECUTING;
                mSeekCondition.signal();
            } else if (mState == PAUSED) {
                POSTPROC_DERIVED_LOGV("ppreadloop: In pause state\n");
                mPausedCondition.wait(mLock);
                POSTPROC_DERIVED_LOGV("ppreadloop: Out of pause state\n");
            }
        ...
        if (readError != OK) {
            POSTPROC_DERIVED_LOGV("%s:  read returned 0x%X", __func__, readError);
            continue;
        } else {
            ...
            /* This block is to avoid post processing of the current buffer if seek is issued */
            {
                Mutex::Autolock autoLock(mLock);
                if (mState == SEEKING) {
                    POSTPROC_DERIVED_LOGV("ppreadloop:  got seeking after source read, drop the buffers\n");
            ...
            if (!(buffer->meta_data()->findInt32(kKeyColorFormat, &inputFormat))) {
                POSTPROC_DERIVED_LOGV("Setting source input format as %x\n", mSrcFormat);
            ...
            if (needsPostProcessing()) {
                ...
                if (readError != OK) {
                    POSTPROC_DERIVED_LOGV("Release PP buffer as post processing gave an error\n");
                    releasePostProcBuffer(bufferPP);
                    continue;
                }
                bufferToQ = bufferPP;
...
status_t PostProc::findFreePostProcBuffer(MediaBuffer **buffer)
{
    ...
    while (mFreePostProcBuffers.empty()) {
        ...
        POSTPROC_DERIVED_LOGV("Got out of the wait for post proc buffer\n");
        if (mState == SEEKING || mState == STOPPED || mState == PAUSED) {
            return OK;
        }
        if (err != OK) {
            POSTPROC_DERIVED_LOGE("Timed out waiting for post proc buffers\n");
            return err;
        }
    }
...
void PostProc::queueBuffer(MediaBuffer *buffer, bool isPostProcBuffer)
{
    ...
    if ((mState == SEEKING) || (mState == STOPPED)) {
        POSTPROC_DERIVED_LOGV("queuebuffer:  In seek/stop state drop the buffer\n");
        if (isPostProcBuffer) {
            flushPostProcBuffer(buffer);
        } else {
            buffer->release();
...
status_t PostProc::notifyNativeWindow()
{
    ...
    POSTPROC_DERIVED_LOGV("%s:  start", __func__);

    status_t err = OK;
    err = mNativeWindow.get()->postProc_perform(POST_PROC_NATIVE_WINDOW_SET_BUFFER_COUNT, mNumBuffers);
    if (err != OK) {
        POSTPROC_DERIVED_LOGE("Post Proc set buffer count failed\n");
        return err;
    }
...
status_t PostProc::dequeuePostProcBufferFromNativeWindow(MediaBuffer **buffer)
{
    ...
    while (!handled) {
        ...
        if (err == NO_MEMORY && buf == NULL) {
            POSTPROC_DERIVED_LOGV("Wait for post proc buffer \n");
            ...
            POSTPROC_DERIVED_LOGE("dequeueBuffer failed: %s (%d)", strerror(-err), -err);
            return err;
        } else {
            handled = true;
        }
...
status_t PostProc::allocatePostProcBuffers()
{
    ...
    if (mNativeWindow == NULL) {
        ...
        if (mIonFd < 0) {
            POSTPROC_DERIVED_LOGE("Failed to open ion device, errno is %d", errno);
            err = mIonFd;
            return err;
        }
        ...
        for(int i = 0; i < mNumBuffers; i++) {
            ...
            if (fd < 0) {
                POSTPROC_DERIVED_LOGE("Failed to allocate memory for linear buffers\n");
                err = fd;
                return err;
            }
            nh->data[0] = fd;
            ...
            POSTPROC_DERIVED_LOGV("handle is :%x\n",nh->data[3]);
            POSTPROC_DERIVED_LOGV("fd is %d\n",fd);

            void * data = mmap(NULL, mBufferSize, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
            if (data == MAP_FAILED) {
                POSTPROC_DERIVED_LOGE("Could not map ion buffer, errno is %s\n", strerror(errno));
            ...
            POSTPROC_DERIVED_LOGV("allocated buffer of size = %d, fd = %d\n", mBufferSize, fd);
...
void PostProc::releasePostProcBuffers()
{
...
#ifdef USE_ION
    if (mNativeWindow == NULL) {
        POSTPROC_DERIVED_LOGV("buffer q size is %d, total number of buffers is %d\n",mPostProcBuffers.size(), mNumBuffers);
    ...
    for (size_t i = 0; i < ppBuffers->size(); ++i) {
        ...
        if (mNativeWindow == NULL) {
            ...
            native_handle_t * nh = const_cast<native_handle_t *>(packet->meta_handle);
            ion_handle * hnd = (ion_handle *)nh->data[3];
            POSTPROC_DERIVED_LOGV("Release post proc buffer with handle : (%x)", nh->data[3]);
            if (munmap((void *)nh->data[4], nh->data[2]) == -1) {
                POSTPROC_DERIVED_LOGE("munmap failed\n");
            }
            close(nh->data[0]);
            int rc = ioctl(mIonFd, ION_IOC_FREE, &hnd);
            if (rc) {
                POSTPROC_DERIVED_LOGE("Free failed with %d!\n",rc);
            }
            native_handle_delete(nh);
            delete packet;
        } else {
...
void PostProc::releaseQueuedBuffers()
{
    ...
    while (!mOutputBuffers.empty()) {
        POSTPROC_DERIVED_LOGV("Releasing queued buffer\n");
        OutputBuffer * outbuffer = *mOutputBuffers.begin();
        if (outbuffer->isPostProcBuffer) {
...
void PostProc::flushPostProcBuffer(MediaBuffer *buffer)
{
    ...
    if (mNativeWindow == NULL) {
        ...
        if (!rendered) {
            POSTPROC_DERIVED_LOGV("Cancel Post Proc buffer as buffer not rendered\n");
            status_t err = mNativeWindow.get()->postProc_cancelBuffer(buffer->graphicBuffer()->getNativeBuffer());

            if (err != OK) {
                POSTPROC_DERIVED_LOGE("Warning Cancel Post Proc buffer failed\n");
            }
        }
    }
    return;
...
MediaBuffer * PostProc::getPostProcBufferByNativeHandle(ANativeWindowBuffer *buf)
{
    ...
    if (ppBuffers->size() < mNumBuffers) {
        POSTPROC_DERIVED_LOGV("Got a new native window buffer : %p", buf);
...
int PostProc::allocateIonBuffer(size_t size, int *handle)
{
    ...
    *handle = (int)fd_data.handle;
    ...
    if (rc) {
        POSTPROC_DERIVED_LOGE("Failed to MAP ion memory\n");
        return -ENOMEM;
    }
    return fd_data.fd;
#else
...
status_t PostProc::getInputFormat()
{
    ...
    POSTPROC_DERIVED_LOGV("Format is %x\n", format);
    mSrcFormat = format;
    return OK;
}
