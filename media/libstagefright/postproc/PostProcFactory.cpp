...
sp<MediaSource> PostProcFactoryCreate(const sp<MediaSource> &decoder, const sp<PostProcNativeWindow> &nativeWindow, const sp<MetaData> &meta, PostProcController **ppController, PostProcType ppType)
{
    ...
    *ppController = NULL;
    ...
    if (ppType == PostProcType2DTo3D) {
        ...
        if (postProcC2D == NULL) {
            POSTPROC_LOGE("Failed to create post processing C2D object");
            return NULL;
        }
        ...
        if (err != OK) {
            postProcC2D.clear();
            POSTPROC_LOGE("Failed to initialize post processing C2D object");
            return NULL;
        }
        ...
        if (postProc2Dto3D == NULL) {
            POSTPROC_LOGE("Failed to create post processing 2d to 3d object");
            postProcC2D.clear();
            return NULL;
        }
        ...
        if (err != OK) {
            POSTPROC_LOGE("Failed to initialize post processing object");
            postProcC2D.clear();
            return NULL;
        }
