...
bool CameraClient::lockIfMessageWanted(int32_t msgType) {
    ...
    while (mMsgEnabled & msgType) {
        if ((msgType==CAMERA_MSG_PREVIEW_FRAME) && (mMsgEnabled&CAMERA_MSG_COMPRESSED_IMAGE)) {
           ALOGE("lockIfMessageWanted(%d): Don't try to acquire mlock if both Preview and Compressed are enabled", msgType);
           return false;
        }
