...
bool AudioFlinger::applyEffectsOn(void *token, int16_t *inBuffer, int16_t *outBuffer, int size, bool force)
{
...
ssize_t AudioFlinger::DirectAudioTrack::write(const void *buffer, size_t size) {
    ...
    if (mFlag & AUDIO_OUTPUT_FLAG_LPA) {
        ...
        mAudioFlinger->applyEffectsOn(static_cast<void *>(this), (int16_t*)buf.localBuf,(int16_t*)buffer,(int)size, true);
        mEffectLock.unlock();
    }
